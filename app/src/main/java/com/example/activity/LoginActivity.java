...
public class LoginActivity extends Activity implements View.OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.rule:// 忘记密码的单击事件
			Toast.makeText(
					...
					"账号必须九位数字，前两位为入学年，如19，代表的是2019年入学;之后三位为专业代码，如001;之后两位为班级代码，如01，最后两位为自己的序号，如01，加起来就是150010101",
		...
		case R.id.btnLogin:// 登录按钮的单击事件
			...
			if ("".equals(editUserName.getText().toString())
				...
				Toast.makeText(getApplicationContext(), "这位同学，学号长度必须为9位哦！",
						Toast.LENGTH_SHORT).show();
	...
	protected void checkPassword() {
		...
		if (!editPassword.getText().toString()
			...
			Toast.makeText(getApplicationContext(), "这位同学，密码和账号是一样的才行哦",
					Toast.LENGTH_SHORT).show();
		} else {
			// 保存
			checkIsRememberPaw();
