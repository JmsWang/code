...
import com.example.utils.HorizontalProgressBarWithNumber;
import com.example.utils.SQLiteDBUtil;
...
public class UserFragment extends BaseFragment implements OnClickListener {
	...
	long currentCount;
	// 这个是进度条
	private com.example.utils.HorizontalProgressBarWithNumber mProgress;
	...
	private int getCounts() {
		// 这里默认自己是19级（默认开学时间是19.9.1）的，之后会传参数，
		Utility util = new Utility();
		pref = getActivity().getSharedPreferences("data", Context.MODE_PRIVATE);
		String name = pref.getString("name", "");
		int year = util.getAccountYear(name);

		// String S19before = "Wed Sep 1 09:22:14 格林尼治标准时间+0800 2019";
		String S19before = "Wed Sep 1 09:22:14 格林尼治标准时间+0800 20" + year;// 这行代码代替了上一行代码，动态的获取用户的账号的年
		Date d1 = new Date();
		SimpleDateFormat sdf1 = new SimpleDateFormat("yy-MM-dd", Locale.CHINA);

		Date dNow = util.formatDate2(d1.toString());
		Date d19 = util.formatDate2(S19before);

		long count = (dNow.getTime() - d19.getTime());
